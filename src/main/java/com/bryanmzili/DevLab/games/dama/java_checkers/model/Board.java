 ...
 */
package com.bryanmzili.DevLab.games.dama.java_checkers.model;
 ...
 * checker board is 8 x 8 (64) tiles, alternating white/black. Checkers are only
 * allowed on black tiles and can therefore only move diagonally. The board is
 * optimized to use as little memory space as possible and only uses 3 integers
 * to represent the state of the board (3 bits for each of the 32 tiles). This
 * makes it fast and efficient to {@link #copy()} the board state.
 * <p>
 * This class uses integers to represent the state of each tile and specifically
 * uses these constants for IDs: {@link #EMPTY},
 * {@link #BLACK_CHECKER}, {@link #WHITE_CHECKER}, {@link #BLACK_KING},
 * {@link #WHITE_KING}.
 * <p>
 * Tile states can be retrieved through {@link #get(int)} and
 * {@link #get(int, int)}. Tile states can be set through {@link #set(int, int)}
 * and {@link #set(int, int, int)}. The entire game can be reset with
 * {@link #reset()}.
 */
public class Board {
    ...
    /**
     * Creates an exact copy of the board. Any changes made to the copy will not
    ...
    /**
     * Sets the ID of a black tile on the board at the specified location. If
     * the location is not a black tile, nothing is updated. If the ID is less
     * than 0, the board at the location will be set to {@link #EMPTY}.
     *
     ...
     * @return the middle point between two points or (-1, -1) if the points are
     * not on the board, are not distance 2 from each other in x and y, or are
     * on a white tile.
     ...
     * @return true if the ID corresponds to a {@link #BLACK_CHECKER} or a
     * {@link #BLACK_KING} checker.
     */
     ...
     *
     * @param id	the ID to check.
     * @return true if the ID corresponds to a {@link #WHITE_CHECKER} or a
     * {@link #WHITE_KING} checker.
     */
     ...
     *
     * @param id	the ID to check.
     * @return true if the ID corresponds to a {@link #BLACK_KING} checker or a
     * {@link #WHITE_KING} checker.
     */
